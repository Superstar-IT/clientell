  ...
  <div class="animated fadeIn frontend-content mb-5 mt-5">
            ...
            <div v-else>
              <template v-if="user.subscription">
                ...
                <b-form-group class="mt-3" label="Payment Details">
                  <b-form-radio-group v-model="selected" :options="options" name="radioInline"></b-form-radio-group>
                </b-form-group>
                <b-row>
                  <b-col sm="6">
                    <b-card body-class="p-3">
                      <template
                        v-if="selected === 'old'"
                      >{{ user.card_brand }} ending with {{ user.card_last_four }}</template>
                      <b-form v-else>
                        <!--<div class="mb-3">
                ...
                <b-btn class="mt-2 d-block" variant="success" @click="resume">
                  <i class="fa fa-check"></i> Resume Subscription
                </b-btn>
              </template>
              <template v-else>
                ...
                <div class="alert alert-warning">
                  <i class="fa fa-warning"></i> Your trial period is up.
                </div>
                <b-row>
                  <b-col sm="10">
                    <b-form>
                      ...
                      >
                        ...
                        >
                          <i
                            class="fa"
                            :class="{'fa-check': !form.busy || checkingCoupon, 'fa-spinner fa-spin': form.busy && !checkingCoupon}"
                          ></i> Subscribe Now
                        </b-btn>
                      </b-form-group>
                    </b-form>
    ...
    <b-modal ref="modalResume" title="Confirmation">
      ...
      <div slot="modal-footer">
        ...
        <b-btn variant="success" @click="confirmResume" :disabled="form.busy">
          <i class="fa" :class="{'fa-check': !form.busy, 'fa-spinner fa-spin': form.busy}"></i> Resume Subscription
        </b-btn>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      App.post(`/api/v1/subscription/resume`, this.form).then(() => {
        this.form.startProcessing();
